...
from tools.sales_forecaster import SalesForecasterTool
...
TOOLS = {
    ...
    "forecast_sales": SalesForecasterTool(),
}
...
5. forecast_sales(file_path, date_column, sales_column, months_to_predict)
   المهمة: تحليل مبيعات شهرية من أي ملف مبيعات وتوقع الأشهر القادمة.
   يمكنه اكتشاف عمود التاريخ وعمود المبيعات تلقائيًا.
   مثال:
   ACTION: forecast_sales
   INPUT: {"file_path": "data/sales.csv", "months_to_predict": 3}
...
SYSTEM_PROMPT = f"""أنت محلل بيانات مبيعات ذكي وكيل (Agentic AI Data Analyst).
